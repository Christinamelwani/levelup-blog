<script>

</script>

<template>
    <div class="byline">
        <div class="byline__inner">
            <img class="byline__img" src="@/assets/images/Ellipse4.png" />
            <div class="byline__wrapper">
                <h3 class="byline__header">
                    By Jennifer Lawrence
                </h3>
                <h4 class="byline__subheader">
                    Thinker and designer
                </h4>
            </div>
        </div>
        <div v-if="$route.name==='article'">
            <svg width="30" height="30" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink">
                <mask id="mask0_55_2108" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="20"
                    height="20">
                    <rect width="20" height="20" fill="url(#pattern0)" />
                </mask>
                <g mask="url(#mask0_55_2108)">
                    <rect width="20" height="20" fill="#495057" />
                </g>
                <defs>
                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlink:href="#image0_55_2108" transform="scale(0.0333333)" />
                    </pattern>
                    <image id="image0_55_2108" width="30" height="30"
                        xlink:href="data:image/png;base64," />
                </defs>
            </svg>
            <svg width="30" height="30" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink">
                <mask id="mask0_55_2107" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="20"
                    height="20">
                    <rect width="20" height="20" fill="url(#pattern0)" />
                </mask>
                <g mask="url(#mask0_55_2107)">
                    <rect width="20" height="20" fill="#495057" />
                </g>
                <defs>
                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlink:href="#image0_55_2107" transform="scale(0.0333333)" />
                    </pattern>
                    <image id="image0_55_2107" width="30" height="30"
                        xlink:href="data:image/png;base64," />
                </defs>
            </svg>
            <svg width="30" height="30" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink">
                <mask id="mask0_55_2106" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="20"
                    height="20">
                    <rect width="20" height="20" fill="url(#pattern0)" />
                </mask>
                <g mask="url(#mask0_55_2106)">
                    <rect width="20" height="20" fill="#495057" />
                </g>
                <defs>
                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlink:href="#image0_55_2106" transform="scale(0.0333333)" />
                    </pattern>
                    <image id="image0_55_2106" width="30" height="30"
                        xlink:href="data:image/png;base64," />
                </defs>
            </svg>
            <svg width="30" height="30" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink">
                <mask id="mask0_55_2105" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="20"
                    height="20">
                    <rect width="20" height="20" fill="url(#pattern0)" />
                </mask>
                <g mask="url(#mask0_55_2105)">
                    <rect width="20" height="20" fill="#495057" />
                </g>
                <defs>
                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlink:href="#image0_55_2105" transform="scale(0.0333333)" />
                    </pattern>
                    <image id="image0_55_2105" width="30" height="30"
                        xlink:href="data:image/png;base64," />
                </defs>
            </svg>
        </div>
    </div>
</template>